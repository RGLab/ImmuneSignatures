...
#### Manuscript Figure 1 - Age Distribution
```{r age-distribution}
pd <- ldply(eset_list, function(sdy){ pData(eset_list[[sdy]]) })
ageCut <- 65
pd$Age.group <- ifelse(pd$Age >= ageCut, paste("Over", ageCut), paste("Under", ageCut))

p <- ggplot(data = pd, 
            aes(x = Study, y=Age.group)) + 
            geom_dotplot(binaxis="y", 
                         fill="#0838FF", 
                         binwidth=1, 
                         stackdir="center", 
                         aes(y = Age)) + 
            ggtitle("Age distribution") +                                                                 ylab("Age") + 
            theme_bw() +
...
plot(p)
p <- ggplot(data = pd, 
            aes(x = Study, fill = paste(Gender, Age.group))) + 
            scale_fill_manual('Groups', values = brewer.pal(4, "Paired")[1:4])

p + 
  geom_boxplot(aes(y = Age)) + 
  ggtitle("Age distribution") + 
  geom_jitter(aes(y = Age), position = position_jitterdodge(), show_guide = FALSE)
...
#### SIGNIFICANT PATHWAYS SELECTION PARAMETERS:
- **FDR.cutoff: ** 0.5
