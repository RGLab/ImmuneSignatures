...
these expression matrices from ImmuneSpace / GEO can be found in the `R/get_GE.R` script within the package.
There are also two cases where data from ImmuneSpace could not be used: SDY80 (hai) and SDY400 (ge).  
...
The SDY400 gene expression data used in the package comes from the authors at Yale University because the "series.txt" file on GEO has different probe-to-gene symbol mapping that cannot be retroactively changed, while the "non-normalized.txt" version on GEO has experiment specific subject ID mappings that were not able to be correctly given ImmuneSpace subject ids at the time of this report's development.
...
for(sdy in studies){
  if(sdy == "SDY400"){
    # SDY400:
    # Using original table because GEO series matrix produces different results 
    # in terms of gene symbols incorporated into qusage analysis, perhaps due 
    # to different normalization method(?), and GEO non-normalized.txt does 
    # not have sample names that can be mapped with available information.
...
#### Adjusting HAI - Figure 3
To provide a more detailed understanding how the hai data adjustment affects the distribution of responders and non-responders, we provide the example of SDY404.  Note that the outlier in Figure 3A. with an extremely high d0 titer is removed from the data before decorrelation.
```{r adjMFC-demo}
...
# plot BEFORE decorrelation
cc <- cor.test(pd$young_fc_norm_max_ivt, pd$young_d0_norm_max, method = "spearman", exact = F)

xrng = range(pd$young_d0_norm_max, na.rm=T)
yrng = range(pd$young_fc_norm_max_ivt, na.rm=T)
...
print("BEFORE DECORRELATION - 3A")
print(sprintf("subjects = %d, Spearman rho = %.2f, p-value = %.2g",
                                      nrow(pd),
                                      cc$estimate,
                                      cc$p.value))
print(plot_grid(beforeHist, 
          beforePlot, 
          ncol = 2, 
          nrow = 1,
          align="h",
...
afterPlot <- plot_size_bubbles(pd$young_d0_norm_max, pd$young_fc_res_max)        +
                      ...
                      theme_bw()                                  + 
                      theme(legend.key = element_blank())        +
                      geom_hline(yintercept = quantile(pd$young_fc_res_max, c(0.3, 0.7), na.rm=T),
                                 col="blue", lty = 2)        +
...
print("AFTER DECORRELATION - 3B")
print(sprintf("subjects = %d, Spearman rho = %.2f, p-value = %.2g",
                                      nrow(pd),
                                      cc$estimate,
                                      cc$p.value))
print(plot_grid(afterHist, 
          afterPlot, 
          ncol = 2, 
          nrow = 1,
          align="h",
...
discPlot <- ggplot(coordDfs$SDY212, aes(x,y)) +
                geom_line(col="#E41A1C") +
                geom_line(data = coordDfs$SDY63, aes(x,y), col="#377EB8") + 
                geom_line(data = coordDfs$SDY404, aes(x,y), col="#4DAF4A") + 
                geom_line(data = coordDfs$SDY400, aes(x,y), col="#984EA3") + 
                geom_line(data = coordDfs$`Meta-Analysis`, aes(x,y), col="black") +
                xlim(-1,1) +
...
t01  <- dcast(tcData[timepoint.day %in% c(0,1)], 
              rn~timepoint.day, 
              value.var='score')
t01p <- t.test(t01$`0`, 
               t01$`1`, 
...
timeSDY80 <- lapply(unique(tcData$timepoint.day),
                    function(t)
                      t.test(tcData[timepoint.day == t & resp=='Low responder']$score,
                             tcData[timepoint.day == t & resp=='High responder']$score))
names(timeSDY80) <- unique(tcData$timepoint.days)
...
#### Figure 4
Forest plot of gene up/down reg from discovery cohort
```{r heatmaps}
# need up/down reg for each selected gene for each study
```

#### Figure 6
```{r sdy80-singleGene-analysis}
...
gem$pheno$resp <- c("Low responder",
                    "Moderate responder",
                    "High responder")[ gem$class + 1 ]

tcData <- gem$pheno[ , colnames(gem$pheno) %in% c("resp", "timepoint.day", "score") ]
...
```{r get-old-res}
sgaResOld <- singleGeneAnalysis(adjEsetList = oldEsets, cohort = "old")
```

## STEP 4: CROSS-META-ANALYSES FIGURES

#### Figure 5
Combining the results of the module intensity and single gene analysis, Figure 5 presents heat maps of both.
```{r heatmaps}
# A - Young Module
...
#### Effect Sizes - Figure 8
```{r old-vs-young-gene-effect}
# Single Gene --------------------------------------------------------------------
#create input dataset
sga <- as.data.table(merge(sgaResOld$disc$metaAnalysis$pooledResults,
                           sgaResYng$disc$metaAnalysis$pooledResults,
                           by = 0))

#create figure
newCOrrPlotBW <- ggplot(sga,
  ...
  geom_point(data = sga[grep("^(RAB24|GRB2|DPP3|ACTB|MVP|DPP7|ARCP4)$", sga$Row.names),],
             aes(x = effectSize.x,y = effectSize.y),
             color = 'gray20',
             size  = 3,
             shape = 15) +
  geom_point(data = sga[grep("^(PTPN22|PURA|SP4)$", sga$Row.names),],
             aes(x = effectSize.x,y = effectSize.y),
             color = 'gray20',
             size  = 3,
             shape = 17) +
...
#create input dataset
gma <- as.data.table(merge(yng_res$disc,
                           old_res$disc,
                           by = 0))

#create figure
newCOrrPlotBW <- ggplot(gma,
  aes(y = effectSize.y, x = effectSize.x)) +
  geom_vline(xintercept=0,size=0.1, colour="black")   +
  geom_hline(yintercept=0,size=0.1, colour="black")   +
  geom_point(color = 'gray65')                        +
  ...
  theme(axis.title        = element_text(size=16,face="bold"),
        ...
        legend.text       = element_text(size = 15,face="bold"),
        legend.background = element_rect(colour = "lightgray")) +
  geom_point(data = gma[grep("^(RAB24|GRB2|DPP3|ACTB|MVP|DPP7|ARCP4)$", gma$Row.names),],
             aes(x = effectSize.x,y = effectSize.y),
             color = 'gray20',
             size  = 3,
             shape = 15) +
  geom_point(data = gma[grep("^(PTPN22|PURA|SP4)$", gma$Row.names),],
             aes(x = effectSize.x,y = effectSize.y),
             color = 'gray20',
             size  = 3,
             shape = 17) +
