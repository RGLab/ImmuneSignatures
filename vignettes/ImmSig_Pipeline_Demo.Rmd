...
for(sdy in studies){
  start <- Sys.time()
  message(paste0("Now assembling study - ", sdy))
  con <- CreateConnection(sdy)
  
  # Get expressionSet object with normalized gene expression values and map SampleNames.
  ...
  # NOTE 3: SDY212 with unexplained dup biosamples; both "BS694717" are removed.
  if(!(sdy %in% c("SDY400","SDY67"))){
    mats <- con$GeneExpressionMatrices()
    eset <- con$getGEMatrix(mats$name, summary = T)
    if(sdy == "SDY212"){ eset <- eset[ , -which(sampleNames(eset) == "BS694717") ] }
  ...
  eset_list[[sdy]] <- eset
  finish <- Sys.time()
  message(paste0("Runtime: ", (finish - start)))
}
```
