...
vignette: >
  ...
  %\VignetteIndexEntry{ImmuneSignatures Report}
---
...
#### Package Notes:
In order to run this vignette locally, you would first need to install the following Bioconductor packages by hand: Biobase, qusage, and MetaIntegrator. Then install `ImmuneSignatures` via `devtools::install_github("rglab/ImmuneSignatures", build_vignettes=T)` and view the vignette with `vignette("ImmSig_Pipeline_demo", package="ImmuneSignatures")`.  
...
con <- CreateConnection("IS1", onTest = TRUE)

for(sdy in studies){
  gTerm <- ifelse(sdy == "SDY80", "pH1N1", sdy)
  mats <- con$cache$GE_matrices$name[ grep(gTerm, con$cache$GE_matrices$name) ]
  
  # ---------------------------GET GENE EXPRESSION DATA------------------------------
  # "ImmSig" annotation is particular to time the microarrays were read and developed
  ...
  # 1. Prior to normalization, probe with NAs must be taken out before normalization. 
  ...
  if(sdy %in% c("SDY63", "SDY404")){
    ...
    # gene_set actually has some as lower, e.g. 'C10orf11', but to 
