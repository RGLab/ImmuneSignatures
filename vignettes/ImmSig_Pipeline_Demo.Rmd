...
```{r adjMFC-demo, fig.width=11, fig.height=9}
...
beforePlot <- plot_size_bubbles(pd$young_d0_norm_max, pd$young_fc_norm_max_ivt)        +
                      ...
                      ggtitle(bquote(atop(.("Before Correlation"), atop(italic(.(plot.subtitle)), "")))) +
...
afterPlot <- plot_size_bubbles(pd$young_d0_norm_max, pd$young_fc_res_max)        +
                      scale_size_area(max_size = 8)               + 
                      ggtitle(bquote(atop(.("After Correlation"), atop(italic(.(plot.subtitle)), "")))) +
                      xlab("Day 0 Titer Score")                   +
                      ylab("") +
                      xlim(-0.5, 7.5)                              +
                      ylim(-2.75, 2.75)                              +
...
sgaResYng <- singleGeneAnalysis(adjEsetList = yngEsets, cohort = "young")
```
...
#### Figure 4
Forest plot of gene up/down reg from discovery cohort. Code developed by F. Vallenia @ Stanford.
```{r sga-disc-forestplot}
customForestPlot <- function(geneName, metaObj, summaryTbl){
  ...
  titleN <- paste(geneName,
                  "\nES =",
                  round(summaryTbl[ rn == i ]$effectSize, 2),
                  " p =",
                  formatC(summaryTbl[ rn == i ]$effectSizePval, 3))
...
```
![Single Gene Analysis Forest Plots for Discovery Group](fig4.pdf)
...
Figure 8A compares p-values of each gene in older and younger cohorts, while figure 8B does the same for pathway-activity of the gene set modules.
```{r old-vs-young-gene-effect, fig.height=4, fig.width=8}
# Single Gene --------------------------------------------------------------------
# input
sga <- as.data.table(merge(sgaResOld$disc$metaObj$metaAnalysis$pooledResults,
                           sgaResYng$disc$metaObj$metaAnalysis$pooledResults,
                           by = 0))
...
effCorrGMA<- ggplot(gma,
  ...
  theme(axis.title        = element_text(size=16,face="bold"),
        ...
        legend.text       = element_text(size = 15,face="bold"),
        legend.background = element_rect(colour = "lightgray")) +
  geom_point(data = gma[ grep("M10.0|M10.1|M33|M42|M54|M72.0|M72.1|M191", 
                             gma$Row.names), ],
             aes(x = pathwayActivity.x, y = pathwayActivity.y),
             color = 'gray20',
             size  = 3,
             shape = 15) +
  geom_point(data = gma[grep("M198|M223", gma$Row.names),],
             aes(x = pathwayActivity.x,y = pathwayActivity.y),
             color = 'gray20',
             size  = 3,
             shape = 17) +
